...
import vf.sudoku.model.grid.{Grid, Slot}
...
object CenterRingAndCornersMatcher
{
	...
	private def cornerGridsIn(sudoku: SudokuState) = sudoku.grids.filter { _.slots.exists { slot => cornerBounds.exists {
		_.contains(slot.position.map { _.toDouble }) } } }
	
	private def centerGridsIn(sudoku: SudokuState) = sudoku.grids.filterNot { grid =>
		innerCenterBounds.contains(grid.position.map { _.toDouble }) }
	
	private def cornerLinesIn(sudoku: SudokuState) = Axis2D.values.flatMap { axis => sudoku.slotLinesAlong(axis).filter {
		_.slots.exists { slot => cornerBounds.exists { _.contains(slot.position.map { _.toDouble }) } } } }
	
	private def centerLinesIn(sudoku: SudokuState) = Axis2D.values.flatMap { axis =>
		sudoku.slotLinesAlong(axis).filter { _.slots.exists { slot => outerCenterBounds.contains(slot.position.map { _.toDouble }) } } }
	...
	private def centerSlotsIn(grids: Vector[Grid]) = grids.flatMap { _.slots.filter { slot =>
		outerCenterBounds.contains(slot.position.map { _.toDouble }) } }
	
	private def centerRingIn(sudoku: SudokuState) = sudoku.grids.flatMap { grid =>
		grid.slots.filter { slot =>
			val position = slot.position.map { _.toDouble }
			outerCenterBounds.contains(position) && !innerCenterBounds.contains(position)
		}
	}
